...
export default class RegistrationForm {
    constructor() {
        ...
        this.email.previouValue = ''
        this.password = document.querySelector('#password-register')
        this.password.previouValue = ''
    ...
    passwordAfterDelay(){
        if(this.password.value.length < 8){
            this.showValidationError(this.password, 'Password must be at least 8 chracters.')
        }
    }
    ...
    usernameAfterDelay(){
        ...
        if(!this.username.errors) {
            axios.post('/doesUsernameExist', {_csrf: this._csrf, username: this.username.value}).then((response) => {
                if(response.data) {
                    this.showValidationError(this.username, 'That username already exists.')
                    this.username.isUnique = false
                } else {
